...
const getCotacaoAPI = (data) => axios.get(getUrl(data)); //me retorna uma promise
const extractCotacao = (res) => res.data.value[0].cotacaoVenda;
const getToday = () => {
    const today = new Date()
    return (today.getMonth()+1) + '-' + (today.getDate()) + '-' + (today.getFullYear()) //month+1 pq a bosta do javascript faz os meses de 0-11
    
}

const getCotacao = async () => {
	try { //se acontecer um erro com o link, ele aconteceria no "await" então, preciso fazer algo antes, por isso o try
		...
		const res = await getCotacaoAPI(today); //não existe cotação no findi nem feriados
